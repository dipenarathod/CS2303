...
void Production::production(){
	...
	prod->welcomeScreen ();
	prod->systemMessage ("                            Hit <ENTER> to continue!\n");
	system ("cls");
	...
	board1->initializeGameBoard (playerOneGameBoard);
	board1->initializeGameBoard (playerTwoGameBoard);
	...
	switch (option) {
	case 1: board1->manuallyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
	break;
	case 2: board1->randomlyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
	break;
	default:
	...
	while (TRUE) {
		...
		case PLAYER_ONE:
			...
			board1->printGameBoard (playerTwoGameBoard, FALSE);
			printf ("> PLAYER 1'S TURN\n");fflush(stdout);

			/* Prompts user to enter target */
			do {
				target = ship1->getTarget ();
				shot = ship1->checkShot (playerTwoGameBoard, target);
		...
		case PLAYER_TWO:
			...
			board1->printGameBoard (playerOneGameBoard, TRUE);
			...
			if (targetMode) {
				...
				do {
					if (cardinals[NORTH]) {        /* NORTH */
						...
						if (ship1->checkShot (playerOneGameBoard, targetTemp) != -1 && north >= 0) {
						...
						if (ship1->checkShot (playerOneGameBoard, targetTemp) != -1 && south <= 9) {
						...
						if (ship1->checkShot (playerOneGameBoard, targetTemp) != -1 && west >= 0) {
							cardinals[WEST] = TRUE;
						}
						...
						if (ship1->checkShot (playerOneGameBoard, targetTemp) != -1 && east <= 9) {
							cardinals[EAST] = TRUE;
						}
					...
					shot = ship1->checkShot (playerOneGameBoard, target);
			...
			if (huntMode) {
				...
				do {
					target.row = board1->getRandomNumber (0, 9);
					target.column = board1->getRandomNumber (0, 9);
			...
			if (shot == 1) {
				...
				if (cardinals[NORTH] == TRUE) {  /* NORTH */
					...
					ship1->checkBoundsOfCardinal (cardinals, north, NORTH);
					targetTemp = target;
					targetTemp.row = north;
					if (ship1->checkShot (playerOneGameBoard, targetTemp) == -1)
						cardinals[NORTH] = FALSE;
				}

				if (cardinals[SOUTH] == TRUE) {  /* SOUTH */
					south = targetAI.row + 1;
					ship1->checkBoundsOfCardinal (cardinals, south, SOUTH);
					targetTemp = target;
					targetTemp.row = south;
					if (ship1->checkShot (playerOneGameBoard, targetTemp) == -1)
				...
				if (cardinals[WEST] == TRUE) {   /* WEST */
					...
					ship1->checkBoundsOfCardinal (cardinals, west, WEST);
					targetTemp = target;
					targetTemp.column = west;
					if (ship1->checkShot (playerOneGameBoard, targetTemp) == -1)
				...
				if (cardinals[EAST] == TRUE) {   /* EAST */
					...
					ship1->checkBoundsOfCardinal (cardinals, east, EAST);
					targetTemp = target;
					targetTemp.column = east;
					if (ship1->checkShot (playerOneGameBoard, targetTemp) == -1)
		...
		if (shot == 1) { /* HIT */
			...
			if (player == 1)
				hasAShipSunked = ship1->checkSunkShip (sunkShip, !player, shipSymbol, outStream);
			else
				ship1->checkSunkShip (sunkShip, !player, shipSymbol, outStream);
		...
		if (player == 0) /* If Player 1 then update player 2's gameboard */
			board1->updateGameBoard (playerTwoGameBoard, target);
		else             /* If Player 2 then update player 1's gameboard */
			board1->updateGameBoard (playerOneGameBoard, target);

		/* Determins if there is a winner based on number of hits */
		if (ship1->isWinner (players, player)) {
			printf ("\n> Player %d wins!\n", player + 1);fflush(stdout);
